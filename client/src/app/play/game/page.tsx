...
// Helper function to format seconds Left into MM:SS format
...
function PlayerGameContent() {
  ...
  const handleSelectDecision = (decisionType: string) => {
    if (!socket || !isConnected || !state.myTeam) return;
  ...
  const handleAnswerQuiz = (optionIdx: number) => {
    if (!socket || !isConnected || !state.myTeam || !state.activeQuiz) return;
    ...
    // Emit to server
    socket.emit("player:quiz-answer", {
      roundId: "current",
      teamId: state.myTeam.id,
    ...
    return (
      <div className="max-w-4xl mx-auto py-8 space-y-8 animate-fade-in">
        <div className="text-center space-y-3">
          <Badge className="bg-amber-500/10 text-amber-400 border border-amber-500/25 uppercase px-4 py-1 tracking-widest text-[10px] font-bold">
            GAME OVER
          </Badge>
        ...
        <Card className="border-slate-800 bg-slate-900/40 backdrop-blur shadow-2xl">
          <CardHeader>
              ...
              <div className="space-y-3">
                {leaderboard.map((item, idx) => {
                  ...
                  return (
                    <div
                      key={item.teamId}
                      className={`p-4 rounded-lg border flex items-center justify-between transition-colors ${isOwnTeam ? 'bg-amber-500/10 border-amber-500/50 shadow-md' : 'bg-slate-950/40 border-slate-800/80'}`}
                    >
                      <div className="flex items-center gap-4">
                        <span className={`w-8 h-8 rounded-full flex items-center justify-center font-black font-mono text-sm ${idx === 0 ? 'bg-amber-500 text-slate-950 shadow-lg' : idx === 1 ? 'bg-slate-300 text-slate-950' : idx === 2 ? 'bg-amber-700 text-slate-100' : 'bg-slate-800 text-slate-400'}`}>
                          {item.rank}
                        </span>
                        <div>
                          ...
                          <span className="text-[10px] text-slate-500 block">Thị phần: {item.marketShare}% | Rủi ro: {item.monopolyRisk}%</span>
                        </div>
                      </div>
                      <div className="text-right">
                        <span className="text-lg font-black text-amber-500 font-mono">{item.totalScore.toLocaleString()}</span>
          ...
          <CardFooter className="justify-center border-t border-slate-800/60 pt-6">
            <Button
              onClick={() => {
                localStorage.clear();
                router.push("/");
              ...
              variant="outline"
              className="border-slate-800 hover:bg-slate-800 hover:text-slate-100 font-bold"
            >
              Quay lại Trang chủ
            </Button>
          </CardFooter>
    ...
    <div className="space-y-8 max-w-5xl mx-auto py-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-800">
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-black text-slate-100">Vòng {currentRound}/{totalRounds}</h1>
            <Badge variant="outline" className="border-amber-500/20 text-amber-500 bg-amber-500/5 text-[10px] uppercase font-mono px-2 py-0.5">
              Pha: {phase === "decision" ? "Quyết định" : phase === "quiz" ? "Trắc nghiệm" : phase === "narration" ? "Báo cáo" : phase === "results" ? "Kết quả" : phase}
            </Badge>
          </div>
          <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">
            Mã phòng: <span className="text-amber-500 font-mono select-all">{roomCode}</span>
          </p>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="text-right">
            <span className="text-[10px] font-bold text-slate-500 uppercase block">Thời gian còn lại</span>
            <span className={`text-2xl font-black font-mono leading-none ${roundTimeLeft <= 10 ? 'text-rose-500 animate-pulse' : 'text-amber-400'}`}>
              {formatTime(roundTimeLeft)}
            </span>
          </div>
          <Badge variant={isConnected ? "default" : "destructive"} className="text-xs py-1">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
                    ...
                    {availableDecisions.map((decision) => {
                      ...
                      return (
                        <button
                          ...
                          className={`p-4 rounded-xl border text-left transition-all flex flex-col justify-between h-40 ${isSelected ? 'border-amber-500 bg-amber-500/10 shadow-lg' : 'border-slate-800 bg-slate-950/40 hover:border-slate-700 hover:bg-slate-900/40'}`}
                        >
                          <div className="space-y-1">
                            <span className="text-[9px] font-bold uppercase tracking-wider text-slate-500">
                              Quyết định
                            </span>
                            <h4 className="font-extrabold text-slate-200 text-sm leading-tight">
                              {decision.nameVi || decision.type}
                            </h4>
                            <p className="text-[11px] text-slate-400 line-clamp-3 mt-1 leading-normal font-medium">
          ...
          {phase === "event" && (
            <Card className="border-slate-800 bg-slate-900/40 backdrop-blur shadow-xl relative overflow-hidden">
              <CardHeader>
            ...
            <Card className="border-slate-800 bg-slate-900/40 backdrop-blur shadow-xl">
              ...
              <CardContent className="space-y-4">
                ...
                {narration.type === "education" && (
                  <div className="p-3 rounded border border-blue-500/25 bg-blue-500/5 text-xs text-blue-400">
          ...
          {phase === "quiz" && (
            <Card className="border-slate-800 bg-slate-900/40 backdrop-blur shadow-xl">
              <CardHeader>
                <Badge className="bg-blue-500/10 text-blue-400 border border-blue-500/25 w-fit text-[9px] font-extrabold uppercase">
                  ...
                  <div className="grid gap-3">
                    {activeQuiz.options.map((option, idx) => (
                      <button
                        ...
                        className={`p-4 rounded-lg border text-left transition-all text-xs font-bold ${activeQuiz.answered ? 'bg-slate-950/30 border-slate-900 text-slate-500' : 'bg-slate-950/60 border-slate-800 text-slate-300 hover:border-blue-500/50 hover:bg-slate-900/60'}`}
                      >
                        <span className="text-blue-400 font-mono mr-2">{String.fromCharCode(65 + idx)}.</span>
                        {option}
                      </button>
                    ))}
                  </div>
                ) : (
                  <p className="text-xs text-slate-500 text-center py-8">Chưa có thử thách trắc nghiệm nào hoạt động.</p>
                )}
              </CardContent>
            </Card>
          )}
          ...
          {phase === "results" && (
            <Card className="border-slate-800 bg-slate-900/40 backdrop-blur shadow-xl">
              ...
              <CardContent className="space-y-4">
                <p className="text-xs text-slate-400 leading-relaxed text-center py-12">
        ...
        <div className="space-y-6">
          ...
          <Card className="border-slate-800 bg-slate-900/60 backdrop-blur shadow-xl">
            <CardHeader className="border-b border-slate-800/60 pb-4">
              ...
              <CardTitle className="text-2xl font-black text-slate-100 flex items-center gap-2">
                <span className="text-amber-500 font-mono">#{myTeamNumber}</span>
            ...
            <CardContent className="pt-6 space-y-4">
              ...
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-500 font-bold uppercase">{vi.stats.money}</span>
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs font-bold leading-none">
                  <span className="text-slate-500 uppercase">{vi.stats.marketShare}</span>
                  <span className="text-slate-200">{myTeam?.marketShare || 0}%</span>
                </div>
              ...
              {/* Technology */}
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-500 font-bold uppercase">{vi.stats.technology}</span>
                <span className="font-extrabold text-slate-200">{myTeam?.technology || 0}</span>
              </div>

              {/* Reputation */}
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-500 font-bold uppercase">{vi.stats.reputation}</span>
                <span className="font-extrabold text-slate-200">{myTeam?.reputation || 0}</span>
              </div>

              {/* Monopoly Risk */}
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs font-bold leading-none">
                  <span className="text-slate-500 uppercase">{vi.stats.monopolyRisk}</span>
                  <span className="text-rose-400">{myTeam?.monopolyRisk || 0}%</span>
                </div>
                <Progress value={myTeam?.monopolyRisk || 0} className="h-1.5 bg-rose-950/30" indicatorColor="bg-rose-500" />
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-slate-800 bg-slate-900/40 backdrop-blur shadow-xl">
            ...
            <CardContent className="pt-4 max-h-[300px] overflow-y-auto">
              {allTeams.filter((t) => t.id !== myTeam?.id).length === 0 ? (
                <p className="text-[10px] text-slate-500 text-center py-6">Không có đối thủ cạnh tranh nào trong phòng.</p>
              ) : (
                <div className="space-y-3">
                  {allTeams
                    ...
                    .map((team) => (
                      ...
                      >
                        <div>
                          <h5 className="font-bold text-slate-300">#{team.teamNumber} {team.name}</h5>
                          <span className="text-[9px] text-slate-500 block">Thị phần: {team.marketShare}%</span>
                        </div>
...
export default function GamePage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between relative overflow-hidden">
      {/* Soft color backdrops */}
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden opacity-20">
        <div className="absolute top-1/4 left-1/4 w-[500px] h-[500px] rounded-full bg-blue-600 blur-[130px]" />
        <div className="absolute bottom-1/4 right-1/4 w-[400px] h-[400px] rounded-full bg-amber-500 blur-[120px]" />
      </div>
